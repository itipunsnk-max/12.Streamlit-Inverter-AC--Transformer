    ...
    *,
    ...
    for record in ampacity_records:
        ...
        if cable is None:
            ...
            cable.material is None or cable.material.casefold() != material.casefold()
        ):
            reasons.append(
                "conductor material is missing"
                if cable.material is None
                else "conductor material does not match"
            )
        elif insulation is not None and (
            cable.insulation is None or cable.insulation.casefold() != insulation.casefold()
        ):
            reasons.append(
                "insulation is missing"
                if cable.insulation is None
                else "insulation does not match"
            )
        if reasons:
    ...
    if ranked:
        ...
        source_ids = tuple(
            dict.fromkeys(
                (
                    *selected_cable.metadata.source_ids,
                    *selected_record.metadata.source_ids,
                )
            )
        )
        findings.extend(
...
def select_pe_conductor(
    phase_cross_section_mm2: DecimalLike | None,
    ...
    *,
    ...
    if phase_cross_section_mm2 is None or (
        isinstance(phase_cross_section_mm2, str)
        and (
            not phase_cross_section_mm2.strip()
            or phase_cross_section_mm2.strip().upper().startswith("#REF")
        )
    ):
        missing = Finding(
            "PE_CSA_MISSING",
            "Phase conductor CSA is blank or a broken reference; PE lookup is not assessed.",
            FindingSeverity.REVIEW,
            VerificationStatus.UNKNOWN,
            field="phase_cross_section_mm2",
        )
        decision = DecisionRecord(
            stable_decision_id("PE-LOOKUP", "MISSING"),
            "wiring",
            "PE-LOOKUP",
            RULES.get("PE-LOOKUP").version,
            VerificationStatus.UNKNOWN,
            (TraceValue("phase_cross_section", None, "mm²"),),
            selected_values=(TraceValue("pe_cross_section", None, "mm²"),),
            findings=(missing,),
        )
        return PESelection(AssessmentStatus.MISSING, Decimal("0"), None, (missing,), decision)
    ...
    if matches:
        ...
        source_ids = rule.metadata.source_ids
        ...
        if pe_size is None:
            ...
            findings.append(
                Finding(
                    "PE_SIZE_MISSING",
                    "Exact PE mapping exists but its PE CSA is missing; no value was inferred.",
                    FindingSeverity.REVIEW,
                    verification_status,
                    source_ids,
                    field="pe_cross_section_mm2",
                )
            )
        else:
            status = AssessmentStatus.PASS
    ...
    *,
    ...
    if cable_result.cable_id is None:
        ...
        pe_result = select_pe_conductor(
            ...
            pe_rules,
            phase_material=selected.material or "",
            pe_material=material or selected.material or "",
        )
    not_assessed = (
        Finding(
...
def _missing_conduit_allocation(
    finding: Finding,
    *,
    cable_count: int,
    cable_od_values: tuple[DecimalLike | None, ...],
) -> ConduitAllocation:
    """Return a traceable missing result without converting unknown OD to zero."""

    inputs = (
        TraceValue("cable_count", cable_count),
        *tuple(
            TraceValue(
                f"cable_{index + 1}_outside_diameter",
                Decimal(str(value)) if isinstance(value, float) else value,
                "mm",
            )
            for index, value in enumerate(cable_od_values)
        ),
    )
    decision = DecisionRecord(
        stable_decision_id("COND-FILL", "MISSING", cable_count, *cable_od_values),
        "wiring",
        "COND-FILL",
        ...
        inputs,
        selected_values=(
            TraceValue("conduit_id", None),
            TraceValue("conduit_count", 0),
        ),
        findings=(finding,),
    )
...
def allocate_conduits(
    cable_outside_diameters_mm: Sequence[DecimalLike | None],
    ...
    *,
    ...
    raw_diameters = tuple(cable_outside_diameters_mm)
    if not raw_diameters:
        return _missing_conduit_allocation(
            Finding(
                "BLANK_CABLE_ROW",
                "No physical cable rows were supplied; conduit allocation is missing.",
                FindingSeverity.REVIEW,
                VerificationStatus.UNKNOWN,
                field="cable_outside_diameters_mm",
            ),
            cable_count=0,
            cable_od_values=raw_diameters,
        )
    missing_od = tuple(
        (index, value)
    ...
    if missing_od:
        broken = any(
            ...
            for _, value in missing_od
        )
        return _missing_conduit_allocation(
            Finding(
                "CABLE_OD_MISSING",
                (
                    "One or more cable outside diameters are blank or a broken workbook "
                    "reference (#REF!); conduit fill is not assessed."
                    if broken
                    else (
                        "One or more cable outside diameters are missing; "
                        "conduit fill is not assessed."
                    )
                ),
                FindingSeverity.REVIEW,
                VerificationStatus.UNKNOWN,
            ...
            cable_count=len(raw_diameters),
            cable_od_values=raw_diameters,
        )
    diameters = tuple(
        require_positive(
            cast(DecimalLike, value),
            f"cable_outside_diameters_mm[{index}]",
        )
        for index, value in enumerate(raw_diameters)
    )
