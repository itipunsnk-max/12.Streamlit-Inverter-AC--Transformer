...
def export_project_excel(package: ProjectPackage | Mapping[str, Any]) -> bytes:
    ...
    # Materialize a deep primitive copy before opening the workbook.  A caller
    # cannot change workbook contents by mutating a source mapping while the
    # export is being assembled; the normal delivery path supplies the frozen
    # ProjectPackage created from one ReferenceSnapshot.
    if isinstance(package, ProjectPackage):
        payload = package.to_payload()
    else:
        primitive = to_primitive(package)
        if not isinstance(primitive, Mapping):
            raise TypeError("project package must serialize to an object")
...
def _project_rows(payload: Mapping[str, Any]) -> tuple[tuple[Any, Any], ...]:
    ...
    rows.extend(_flatten_rows(payload.get("hashes", {}), prefix="hashes"))
    rows.extend(_flatten_rows(payload.get("metadata", {}), prefix="metadata"))
    warnings = payload.get("warnings", [])
    overrides = payload.get("overrides", [])
    reconciliation = payload.get("reconciliation", [])
    rows.append(("audit.warning_count", len(warnings) if isinstance(warnings, list) else 0))
    rows.append(("audit.override_count", len(overrides) if isinstance(overrides, list) else 0))
    rows.extend(_flatten_rows(warnings, prefix="warnings"))
    rows.extend(_flatten_rows(overrides, prefix="overrides"))
    rows.extend(_flatten_rows(reconciliation, prefix="reconciliation"))
    return tuple(rows)
